...
function evaluateBSpline(controlPoints, knots, order, t)
{
    const p = order;
    const m = knots.length - 1;
    let ub = Math.max(knots[p],Math.min(t,knots[m-p]));
    let k=p;
    while(true)
    {
        if(knots[k+1] >= ub) break;
        k++;
    }
    let d=[];
    for(let i=0;i<=p;i++)
    {
        let cp = controlPoints[k-p+i];
    ...
    for(let r=1;r<=p;r++)
    {
        for(let i=p;i>=r;i--)
        {
            let denominator = knots[i+1+k-r] - knots[i+k-p];
            let alpha=0;
            if(Math.abs(denominator)>1e-8)
            {
               alpha =(ub - knots[i+k-p])/denominator;
            }
            d[i][0] =(1-alpha)*d[i-1][0]+alpha*d[i][0];
            d[i][1] =(1-alpha)*d[i-1][1]+alpha*d[i][1];
            d[i][2] =(1-alpha)*d[i-1][2]+alpha*d[i][2];
        }
    }
    return d[p];
}

function updateLocalBsplineLine(points)
{
    if(localBsplineLine===null)
    {
        let geometry = new THREE.BufferGeometry();
        geometry.setAttribute('position',new THREE.Float32BufferAttribute(points,3));
        let material =new THREE.LineBasicMaterial({color:0xff8800});
        localBsplineLine =new THREE.Line(geometry,material);
        scene.add(localBsplineLine);
    }
    else
    {
        localBsplineLine.geometry.setAttribute('position',new THREE.Float32BufferAttribute(points,3));
        localBsplineLine.geometry.attributes.position.needsUpdate = true; 
        localBsplineLine.geometry.computeBoundingSphere();
    }
}

function setBsplineTrajectory(msg)
{
    if(!msg.pos_pts ||!msg.knots ||msg.pos_pts.length < 4) return;
    let controlPoints = msg.pos_pts;
    let knots = msg.knots;
    let order = msg.order;
    let start =  knots[order];
    let end = knots[knots.length-order-1];
    let sample_num = Math.max(20,Math.ceil((end-start)/0.05));
    let positions=[];
    for(let i=0;i<sample_num;i++)
    {
        let t =start +(end-start)*i/(sample_num-1);
        let p =evaluateBSpline(controlPoints,knots,order,t);
        positions.push(p[0],p[1],p[2]);
    }
    updateLocalBsplineLine(positions);
}
...
function connectRosbridge(url = wsInput.value.trim(), manual = false) {
  ...
  ros.on("connection", () => {
    ...
    const bsplineTopic = new ROSLIB.Topic({
      ros,
      name:"/planning/bspline",
      messageType:"scan_planner_msgs/msg/Bspline"
    });
    bsplineTopic.subscribe(setBsplineTrajectory);
