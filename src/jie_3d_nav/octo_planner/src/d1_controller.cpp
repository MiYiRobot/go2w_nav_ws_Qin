...
private:
  void onPath(const nav_msgs::msg::Path::SharedPtr msg)
  {
    ...
    // 只有当 require_start_command 开启，且机器人当前“本来就处于静止/未导航状态”时，才需要暂存并等待指令。
    // 如果当前机器人已经处于活跃导航状态（isNavigationActive() 为 true），说明是动态避障重规划，直接无缝更新！
    if (get_parameter("require_start_command").as_bool()) 
    // if (get_parameter("require_start_command").as_bool() && !isNavigationActive())
    {
  ...
  {
    pending_plan_.clear(); 
    clearActivePlan();      //清空等待执行的路径
    publishZeroBurst();     //发送零速度命令
