...
public:
    ...
    last_success_seq_(0)
  {
    ...
    /*
    Qos(1)：队列长度，最多缓存1条消息
    reliable():可靠传输，必须保证收到
    transient_local():后启动的节点，能接收到之前发布过的最后一条消息
    */
    octomap_sub_ = create_subscription<octomap_msgs::msg::Octomap>(   //接收三维地图数据
      octomap_topic, rclcpp::QoS(1).transient_local().reliable(),
      std::bind(&JiePathNode::onOctomap, this, std::placeholders::_1));
    start_sub_ = create_subscription<geometry_msgs::msg::PointStamped>(  //接收起点数据
      start_topic, rclcpp::QoS(1).transient_local().reliable(),
      std::bind(&JiePathNode::onStart, this, std::placeholders::_1));
    goal_sub_ = create_subscription<geometry_msgs::msg::PointStamped>(  //接收终点数据（只有位置）
      goal_topic, rclcpp::QoS(1).transient_local().reliable(),
      std::bind(&JiePathNode::onGoal, this, std::placeholders::_1));
    goal_pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(    //接收终点位置+姿态
      goal_pose_topic, rclcpp::QoS(1).transient_local().reliable(),
      std::bind(&JiePathNode::onGoalPose, this, std::placeholders::_1));
    external_preblocked_sub_ = create_subscription<visualization_msgs::msg::Marker>(  //人工添加不可通行区域
    ...
    edited_occupied_sub_ = create_subscription<visualization_msgs::msg::Marker>(      //手动添加障碍
    ...
    path_pub_ = create_publisher<nav_msgs::msg::Path>(      //发布路径规划数据
      path_topic, rclcpp::QoS(1).transient_local().reliable());
    path_marker_pub_ = create_publisher<visualization_msgs::msg::Marker>( //发布rviz marker数据
      path_marker_topic, rclcpp::QoS(1).transient_local().reliable());
    octomap_pub_ = create_publisher<octomap_msgs::msg::Octomap>(  //发布用户修改后生成的新地图
      octomap_topic, rclcpp::QoS(1).transient_local().reliable());
    preblocked_marker_pub_ = create_publisher<visualization_msgs::msg::Marker>( //发布rviz危险区域（蓝色cube）
      preblocked_marker_topic, rclcpp::QoS(1).transient_local().reliable());
    traversable_marker_pub_ = create_publisher<visualization_msgs::msg::Marker>(  //发布rviz可走区域（A*搜索空间）
      traversable_marker_topic, rclcpp::QoS(1).transient_local().reliable());
    risk_cost_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>(     //用来显示代价地图
      risk_cost_topic, rclcpp::QoS(1).transient_local().reliable());
    get_meta_srv_ = create_service<jie_map_msgs::srv::GetNavigationMapMeta>(    //返回地图信息
      "~/get_meta",
      std::bind(
        &JiePathNode::handleGetMapMeta, this, std::placeholders::_1,
        std::placeholders::_2));
    export_snapshot_srv_ = create_service<jie_map_msgs::srv::ExportNavigationSnapshot>(   //导出当前规划状态
      "~/export_snapshot",
      std::bind(
...
private:
  ...
  //地图格式转换为octomap::OcTree对象，并保存到octree_中
  ...
  //接收用户设定的目标位姿（位置+朝向），保存下来，并立即通知规划器重新规划，使最终路径的终点朝向与用户设定一致。
  ...
  void publishPath(const std::vector<GridIndex> & cells, const std::string & frame_id)
  {
    ...
    // if(cells.empty())
    // {
    //   RCLCPP_WARN(get_logger(), "No point to publish!!!");
    //   return;
    // } 

    // //路径点稀疏化，每隔4个点取1个点
    // const size_t step = 4;
    // std::vector<GridIndex> sparse_cells;
    // for (size_t i = 0; i < cells.size(); i+=step)
    // {
    //   sparse_cells.push_back(cells[i]);
    // }
    // //确保最后一个终点不丢
    // if(!(sparse_cells.back() == cells.back()))
    // {
    //   sparse_cells.push_back(cells.back());
    // }
    // path_msg.poses.reserve(sparse_cells.size());


    for (std::size_t i = 0; i < cells.size(); ++i) {
      const auto & c = cells[i];
      const auto p = gridToWorld(c);  //把octomap voxel转化为世界坐标系下的点
      ...
      if (has_goal_pose_ && i + 1 == cells.size()) {    //只有最后一个点时，使用目标朝向
