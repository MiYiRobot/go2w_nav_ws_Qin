...
if [ $ROS_VERSION = ${VERSION_ROS1} ]; then
    ...
    # 启用了 --symlink-install 并追加了 -DCMAKE_BUILD_TYPE=Release 优化参数
    colcon build --symlink-install --cmake-args -DROS_EDITION=${VERSION_ROS2} -DDISTRO_ROS=${ROS_DISTRO} -DCMAKE_BUILD_TYPE=Release
fi
popd > /dev/null
