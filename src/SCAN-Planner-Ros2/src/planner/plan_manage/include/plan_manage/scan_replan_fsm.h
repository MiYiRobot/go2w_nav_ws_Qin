...
namespace scan_planner
{
  ...
  private:
    ...
    std::string start_navigation_topic_;

    /* planning data */
    bool trigger_, have_target_, have_odom_, have_new_target_; //是否触发规划，是否有目标点，是否有odom数据，是否有新目标点
    bool preset_started_{false};
    bool navigation_started_{false};      //导航启动按键
    bool rviz_height_ready_;    //rviz高度是否更新成功
    bool go2_execution_frozen_;     //控制器是否暂停执行
    bool enable_fail_safe_, need_hover_stop_;   //是否停车等待，重规划连续失败时触发
    FSM_EXEC_STATE exec_state_;   //FSM当前状态
    int continuously_called_times_{0};  //当前状态进入次数
    int replan_fail_count_{0};      //连续重规划失败次数
    int max_replan_fail_count_{1000};
    rclcpp::Time last_freeze_update_time_;    //用于计算冻结时间
    ...
    std::vector<Eigen::Vector3d> active_waypoints_;   //mode2的航点
    std::vector<Eigen::Vector3d> pending_waypoints_;  //mode3的预设航点
    int current_wp_;  //当前导航到第几个航点

    bool flag_escape_emergency_;    //进入急停标志位
