...
def _setup(context):
    ...
    planner_yaml = os.path.join(scan_share, "config", "planner.yaml")  #plan_manage/config/planner.yaml
    controllers_yaml = os.path.join(scan_share, "config", "controllers.yaml")  #plan_manage/config/controllers.yaml
    is_real = _as_bool(LaunchConfiguration("is_real_world").perform(context))  #从外部获取数据
    ...
    # 1：使用RViz2的2D目标点工具选择导航目标   
    # 2：使用参数文件中预设的fsm.waypoints路径点序列导航  
    # 3：订阅initial_path话题获取全局路径，并在局部范围内进行避障
    ...
    if is_real:
        body_pose = "/localization"
        sensor_pose = "/localization"
        cloud = "/cloud_registered_map"
    ...
    # src/scan_planner_node.cpp:  main入口，实例化所有对象
    ...
    if controller_mode == "open_loop":
        ...
        actions.append(     # 订阅b样条轨迹“planning/bspline”，进行轨迹跟踪，输出/cmd_vel
            Node(
                package="scan_planner",
        ...
        if not is_real:
            actions.append( #仅在仿真环境中启动，模拟机器人行走状态和动力学反馈
                Node(
                    package="scan_planner",
...
def generate_launch_description():
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("navi_mode", default_value="3"),
            DeclareLaunchArgument("sensor_type", default_value="lidar"),
            DeclareLaunchArgument("controller_mode", default_value="closed_loop"),
            DeclareLaunchArgument("keypoints_file", default_value=""), #预先设定航点文件路径
            DeclareLaunchArgument("use_gpu", default_value="false"),
            DeclareLaunchArgument("use_pcd_map", default_value="false"),
            DeclareLaunchArgument("pcd_map_file", default_value="/home/luo/go2w_nav_ws_Qin/maps/localization/big_localization_raw.pcd"),
            DeclareLaunchArgument("map_size_x", default_value="60.0"),  #仿真地图大小
