...
public:
  ...
  {
    this->declare_parameter<std::string>("network_interface", "enp2s0");  // 网络接口
    this->declare_parameter<std::string>("cmd_vel_topic", "/cmd_vel");    //速度话题
    this->declare_parameter<std::string>("enable_topic", "/go2w_cmd_enable"); //使能话题
    ...
    enabled_.store(this->get_parameter("start_enabled").as_bool());   //一开始就设置为true
    ...
    const int32_t ret_speed = sport_client_->SpeedLevel(speed_level_); // -1为慢速，0为正常，1为快速
  ...
  ~Go2WCmdBridge()
  {
    safeStop("node shutdown");  //节点关闭时让机器人停下
  }
...
private:
  ...
  {
    enabled_.store(msg->data);   //enabled_一开始就设置为true，没收到消息，就一直处于是能状态
  ...
  void watchdogLoop()
  {
    ...
    if (enabled_.load() && dt > cmd_timeout_) {   
      ...
      RCLCPP_WARN(this->get_logger(),"Timeout: No /cmd_vel received for %.3f seconds. Sending zero velocity to Go2-W.", dt);
    }
  }
